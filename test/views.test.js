...
test('anytime: no-when admin work WITH a project, someday split out, pure-inbox NOT duplicated', () => {
  const db = seed();
  const now = '2026-03-10T08:00:00.000Z';
  // the orphan: active, admin, someday, no project, no due — belongs to
  // Someday now (2026-09-07 split), not Anytime.
  ...
  assert.ok(!ids.includes('orph'), 'someday orphan now lives in Someday, not Anytime');
  assert.ok(!ids.includes('smd1'), 'someday WITH a project now lives in Someday, not Anytime');
  assert.ok(!ids.includes('smd2'), 'parked someday now lives in Someday, not Anytime');
  ...
  assert.deepEqual(new Set(ids), new Set(['tod3']));
});

test('someday: when_type=someday ONLY, with or without a project, delegated stays out', () => {
  const db = seed();
  const now = '2026-03-10T08:00:00.000Z';
  // the orphan (C-bug): active, admin, someday, no project, no due — vanished
  // from every lane before the anytime/someday views existed. MUST show in someday.
  db.prepare(`INSERT INTO tasks (id,title,notes,project_id,status,when_type,when_date,due_date,rank,today_rank,assignee,completed_at,created_at,updated_at)
    VALUES ('orph','someday orphan','',NULL,'active','someday',NULL,NULL,20,NULL,'alex',NULL,?,?)`).run(now, now);
  const ids = run(db, 'someday').map(r => r.id);
  assert.ok(ids.includes('orph'), 'the someday/no-project orphan MUST appear');
  assert.ok(ids.includes('smd1'), 'someday WITH a project appears (even with a due date, C13)');
  assert.ok(ids.includes('smd2'), 'parked someday appears');
  assert.ok(!ids.includes('tod3'), 'no-when task that HAS a project stays in Anytime, not Someday');
  assert.ok(!ids.includes('up1'), 'a future scheduled (when-date) task does NOT appear');
  assert.ok(!ids.includes('inb1'), 'pure-inbox is NOT included');
  assert.ok(!ids.includes('del5'), 'delegated (assignee != admin) stays out');
  assert.deepEqual(new Set(ids), new Set(['orph', 'smd1', 'smd2']));
});
